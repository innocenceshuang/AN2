...
import chapter.android.aweme.ss.com.chapter2.createApplication;
...
//            }
//        }
        createApplication myapp = (createApplication)getApplication();
        if(myapp.getLog()!=null)
        {
            if (myapp.getLog().containsKey(LIFECYCLE_CALLBACKS_TEXT_KEY)) {
                String savedContent = (String) myapp.getLog().get(LIFECYCLE_CALLBACKS_TEXT_KEY);
                mLifecycleDisplay.setText(savedContent);
            }
        }
    ...
    protected void onDestroy() {
        ...
        createApplication myapp=(createApplication)getApplication();
        myapp.writeLog(content);

    }

    private void logAndAppend(String lifecycleEvent) {
        Log.d(TAG, "Lifecycle-saveInstanceState Event: " + lifecycleEvent);
